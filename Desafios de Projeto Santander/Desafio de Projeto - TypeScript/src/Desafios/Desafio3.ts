// // O código abaixo tem alguns erros e não funciona como deveria. Você pode identificar quais são e corrigi-los em um arquivo TS?

// let botaoAtualizar = document.getElementById('atualizar-saldo');
// let botaoLimpar = document.getElementById('limpar-saldo');
// let soma = document.getElementById('soma');
// let campoSaldo = document.getElementById('campo-saldo');

// campoSaldo.innerHTML = 0

// function somarAoSaldo(soma) {
//     campoSaldo.innerHTML += soma;
// }

// function limparSaldo() {
//     campoSaldo.innerHTML = '';
// }

// botaoAtualizar.addEventListener('click', function () {
//     somarAoSaldo(soma.value);
// });

// botaoLimpar.addEventListener('click', function () {
//     limparSaldo();
// });

/**
    <h4>Valor a ser adicionado: <input id="soma"> </h4>
    <button id="atualizar-saldo">Atualizar saldo</button>
    <button id="limpar-saldo">Limpar seu saldo</button>
    <h1>"Seu saldo é: " <span id="campo-saldo"></span></h1>
*/

// Lembretes: 

/** 
    Em todos os casos abaixo de uso do getElementById(), o elemento é potencialmente nulo e ifs são necessários para garantir que seu código vai funcionar da melhor forma.
    No entanto, vão existir situações em que o desenvolvedor vai ter certeza de que o campo está lá e ele pode escrever o código da seguinte maneira:
        document.getElementById('limpar-saldo')!;
    A exclamação no fim é um sinal de que aquele campo não é nulo e que essa função realmente vai trazer algo. Assim, os ifs não são necessários.
*/

// Resposta

// export {} // Para não reclamar de variáveis duplicadas

let botaoAtualizar = document.getElementById('atualizar-saldo');
let botaoLimpar = document.getElementById('limpar-saldo');
let soma = document.getElementById('soma') as HTMLInputElement; // especificado para receber o valor contido no HTML.
let campoSaldo = document.getElementById('campo-saldo');

let caixaTotal = 0;

limparSaldo();

// Função que recebe o número e adiciona ao saldo
function somarAoSaldo(soma: number) {
    if(campoSaldo) {
        caixaTotal += soma;
        campoSaldo.innerHTML = caixaTotal.toString();
        limpaSoma();
    }
}

// Funçao que limpa o campo de saldo
function limparSaldo() {
    if (campoSaldo) {
        caixaTotal = 0;
        campoSaldo.innerHTML = caixaTotal.toString();
    }
}

// Função que apaga o último valor informado
function limpaSoma() {
    soma.value = "";
}

// Checa se o botao não é nulo e recebe o valor de entrada
if(botaoAtualizar){
    botaoAtualizar.addEventListener('click', () => {
        if (soma) {
            somarAoSaldo(Number(soma.value));
        }
    })
}

// Checa se o botao não é nulo e limpa o campo de saldo
if(botaoLimpar){
    botaoLimpar.addEventListener('click', () => {
        limparSaldo();
    })
}

